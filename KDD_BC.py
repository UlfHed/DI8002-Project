...
def main():
    ...
    # Encode label values
    df['label'] = np.where(df['label'] == 'normal.', 'Normal', df['label'])    # Normal is 0.
    df['label'] = np.where(df['label'] != 'Normal', 'Attack', df['label'])    # Not normal (0) is attack.
    df['label_encoded'] = np.where(df['label'] == 'Normal', 0, df['label'])    # Normal is 0.
    ...
    plot_bar_label_frequency(af, 'Normal vs. Attack', 'State', 'Frequency', 'Images/KDD/BC/Label_Frequency_BAR_KDD.png')
    plot_pie_label_frequency(af, 'Normal vs. Attack', 'Images/KDD/BC/Label_Frequency_PIE_KDD.png')

    # Outliers.
    if run_outliers == True:
        df_outliers = outliers(df)
        save('Data/KDD/outlier_data_KDD_BC', df_outliers)
    df_outliers = load('Data/KDD/outlier_data_KDD_BC')
    with pd.option_context('display.max_rows', None, 'display.max_columns', None):
        print('Outlier Data:\n%s'%(df_outliers))
    plot_bar_outliers(df_outliers, 'Feature Outliers (Fliers)', 'Number of Fliers', 'Feature', 'Images/KDD/BC/Feature_Outliers_BAR_KDD.png')

    # PCA Reduction [Can't run scikit-learn PCA in native Python, see Jupyter Notebook].
    pca_components = load('Reduction/KDD/PCA_KDD_BC')
    dfpca = pd.DataFrame()
    ...
    plot_3d(dfpca, 'PCA - 3 Principle Components', 'PC 1', 'PC 2', 'PC 3', 'Images/KDD/BC/PCA_Scatter_KDD.png')

    # TSNE Reduction [Can't run scikit-learn TSNE in native Python, see Jupyter Notebook].
    tsne_result = load('Reduction/KDD/TSNE_KDD_BC')
    dftsne = pd.DataFrame()
    ...
    plot_3d(dftsne, 't-SNE - 3 Dimensions, perplexity=30', 'X', 'Y', 'Z', 'Images/KDD/BC/TSNE_Scatter_KDD.png')
    ...
    for i in range(len(dataset_names)):
        ...
        if run_feature_selection == True and dataset_name == 'Full Dataset':
            ...
            # Bar Graphs. 
            plot_bar_important_features(important_features, 'Feature Importance (%s features) (%s Trees)'%(len(important_features), n_trees), 'Relative Importance', 'Feature', 'Images/KDD/BC/IF_Bar_%s_%s.png'%(len(important_features), n_trees))
            plot_bar_important_features(important_features_1p, 'Feature Importance (%s features) (%s Trees)'%(len(important_features_1p), n_trees), 'Relative Importance', 'Feature', 'Images/KDD/BC/IF_Bar_%s_%s_1p.png'%(len(important_features_1p), n_trees))
        ...
        if run_clustering == True:
            ...
            plot_line_graph(k_values, s_scores, 'Cluster Number (k)', 'Silhouette Score', 'KMEANS - %s - Cluster Number (k) vs. Silhouette Score'%(dataset_name), 'Images/KDD/BC/K_vs_Silhouette_score_%s_KMEANS.png'%(dataset_name.replace(' ', '_')))
            plot_line_graph(k_values, inertias, 'Cluster Number (k)', 'Inertia', 'KMEANS - %s - Cluster Number (k) vs. Inertia'%(dataset_name), 'Images/KDD/BC/K_vs_Inertias_%s_KMEANS.png'%(dataset_name.replace(' ', '_')))
            plot_line_graph(k_values, c_scores, 'Cluster Number (k)', 'Completeness Score', 'KMEANS - %s - Cluster Number (k) vs. Completeness Score'%(dataset_name), 'Images/KDD/BC/K_vs_Completeness_%s_KMEANS.png'%(dataset_name.replace(' ', '_')))
            plot_line_graph(k_values, h_scores, 'Cluster Number (k)', 'Homogeneity Score', 'KMEANS - %s - Cluster Number (k) vs. Homogeneity Score'%(dataset_name), 'Images/KDD/BC/K_vs_homogeneity_%s_KMEANS.png'%(dataset_name.replace(' ', '_')))
            ...
            plot_line_graph(eps_values, n_cluster_values, 'Epsilon', 'Cluster Number', 'DBSCAN - %s - Epsilon vs. Cluster Number'%(dataset_name), 'Images/KDD/BC/eps_vs_cluster_BC_%s_DBSCAN.png'%(dataset_name.replace(' ', '_')))
            plot_line_graph(eps_values, n_noise_values, 'Epsilon', 'Noise', 'DBSCAN - %s - Epsilon vs. Noise'%(dataset_name), 'Images/KDD/BC/eps_vs_noise_BC_%s_DBSCAN.png'%(dataset_name.replace(' ', '_')))
            plot_line_graph(eps_values, c_scores, 'Epsilon', 'Completness Score', 'DBSCAN - %s - Epsilon vs. Completeness Score'%(dataset_name), 'Images/KDD/BC/eps_vs_Completeness_BC_%s_DBSCAN.png'%(dataset_name.replace(' ', '_')))
            plot_line_graph(eps_values, h_scores, 'Epsilon', 'Homogeneity Score', 'DBSCAN - %s - Epsilon vs. Homogeneity Score'%(dataset_name), 'Images/KDD/BC/eps_vs_homogeneity_BC_%s_DBSCAN.png'%(dataset_name.replace(' ', '_')))
        ...
        if run_train == True:
            ...
            save('Models/KDD/BC/%s/DT'%(dataset_name), clf_DT)
            # Support Vector Machine Classifier.
            clf_SVM = SVM_train(X_train, y_train, cv)
            save('Models/KDD/BC/%s/SVM'%(dataset_name), clf_SVM)
            # Multi-Layer Perceptron Classifier.
            clf_MLP = MLP_train(X_train, y_train, cv)
            save('Models/KDD/BC/%s/MLP'%(dataset_name), clf_MLP)
        ...
        # ---------------------------------------- Supervised Learning. ---------------------------------------- #
        clf_DT = load('Models/KDD/BC/%s/DT'%(dataset_name))
        start_time = time.time()
        # Test.
        ...
        # Confusion Matrix.
        plot_cf_matrix(clf_DT, X_test, y_test, ul['label_names'], dataset_name, 'DT', 'Images/KDD/BC/CM_%s_DT.png'%(dataset_name.replace(' ', '_')))

        # ---------------------------------------- 2.2.2. Suport vector Machine Model (SVM). ---------------------------------------- #
        clf_SVM = load('Models/KDD/BC/%s/SVM'%(dataset_name))
        start_time = time.time()
        # Test.
        ...
        print('-'*4+'Prediction & results time: %.4f seconds.'%(time.time()-start_time))
        # Confusion Matrix.
        plot_cf_matrix(clf_SVM, X_test, y_test, ul['label_names'], dataset_name, 'SVM', 'Images/KDD/BC/CM_%s_SVM.png'%(dataset_name.replace(' ', '_')))

        # ---------------------------------------- 2.2.3. Multi-Layer Perceptron - Neural Network Model (MLP). ---------------------------------------- #
        clf_MLP = load('Models/KDD/BC/%s/MLP'%(dataset_name))
        start_time = time.time()
        # Test.
        ...
        print('-'*4+'Prediction & results time: %.4f seconds.'%(time.time()-start_time))
        # Confusion Matrix.
        plot_cf_matrix(clf_MLP, X_test, y_test, ul['label_names'], dataset_name, 'MLP', 'Images/KDD/BC/CM_%s_MLP.png'%(dataset_name.replace(' ', '_')))

        # Plot ROC Curve for all 3 classifiers.
        plot_ROC(y_test, (y_proba_test_DT, y_proba_test_SVM, y_proba_test_MLP), ('DT', 'SVM', 'MLP'), dataset_name, 'Images/KDD/BC/ROC_BC_%s.png'%(dataset_name))
